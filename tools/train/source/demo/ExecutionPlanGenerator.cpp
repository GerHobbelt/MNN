...
Recomputer::Recomputer(shared_ptr<Profiler> profiler_ptr, size_t budget, double thres)
        : profiler(profiler_ptr), budget_mb(budget), threshold(thres) {
    allocator = Allocator();
...
void Recomputer::compute(string ith, bool recompute, set<string> skip_pre_rel) {
    for (auto t : profiler->io_info[stoi(ith)].outputs) {
        allocator.alloc(ith, profiler->tensor_size[t]);
        allocated_tensor.insert(t);
    }
    for (auto p : profiler->resize_info[stoi(ith)]) {
        if (p.first == "alloc") {
            allocator.alloc(p.second, profiler->tensor_size[p.second]);
        }
    }
...
//        debug_print("%s: try release %s\n", ith.c_str(), t.c_str())
    ...
    debug_print("cur_size(%zu)\tbudget_MB=%lu\tbudget_b*thres(%.0lf)\n",
                allocator.current_size(), budget_b >> 20, budget_b * threshold)
    while (allocator.current_size() > budget_b * threshold) {
...
HeuristicAllocator::HeuristicAllocator(shared_ptr<Profiler> profiler_ptr, shared_ptr<Recomputer> recomputer_ptr, bool norecomp)
        : profiler(std::move(profiler_ptr)), budget_mb(recomputer_ptr->budget_mb), noRecompute(norecomp) {}


struct HeuristicAllocator::Tensor {
    string id;
...
bool HeuristicAllocator::load_info_via_exe_seq() {
    ...
    for (int i = 0; i < recomp_seq_info.size(); i++) {
        ...
        heu_info[to_string(i)] =
                make_shared<Tensor>(to_string(i), info.alloc, info.free, profiler->tensor_size[info.id]);
        // resize info
...
bool HeuristicAllocator::overlap(Tensor t1, Tensor t2) {
    return !(t1.free <= t2.alloc || t2.free <= t1.alloc);
}

bool HeuristicAllocator::overlap(MemoryAddress m1, MemoryAddress m2) {
    return !(m1.second <= m2.first || m2.second <= m1.first);
}

bool HeuristicAllocator::mergeable(MemoryAddress m1, MemoryAddress m2) {
    return !(m1.second < m2.first || m2.second < m1.first);
}

void HeuristicAllocator::heuristic_alloc() {
    if (infos.empty()) {
        if (!load_info()) {
...
//        debug_print("alloc for Tensor[%d]\n", i);
        ...
        for (int j = 0; j < i; j++) {
            if (overlap(infos[i], infos[j])) {  // the lifetime of two tensor overlap
//                debug_print("info[%d] overlap with info[%d]\n", i, j);
                auto pos = lower_bound(madd.begin(), madd.end(), heuristic_address[j]) - madd.begin();
                if (pos != 0 && mergeable(madd[pos - 1], heuristic_address[j])) {
                    madd[pos - 1].first = min(madd[pos - 1].first, heuristic_address[j].first);
                    madd[pos - 1].second = max(madd[pos - 1].second, heuristic_address[j].second);
                    pos--;
                } else {
                    madd.insert(madd.begin() + pos, heuristic_address[j]);
                }
...
//        debug_print("\tmadd.size() = %lu\n", madd.size());
        MemoryAddress best(-1, -1);
        if (madd[0].first >= infos[i].size && (best.first == -1 || madd[0].first < best.second - best.first)) {
            best = MemoryAddress(0, madd[0].first);
        }
//        debug_print("\t1: best = <%zu, %zu>\n", best.first, best.second);
        for (int iter = 0; iter + 1 < madd.size(); iter++) {
            if (madd[iter + 1].first - madd[iter].second >= infos[i].size &&
...
//        debug_print("\t2: best = <%zu, %zu>\n", best.first, best.second);
        if (best.first == -1) {
            best = MemoryAddress(madd[madd.size() - 1].second, numeric_limits<size_t>::max());
        }
//        debug_print("\t3: best = <%zu, %zu>\n", best.first, best.second);
        heuristic_address.emplace_back(best.first, best.first + infos[i].size);
    }
...
bool HeuristicAllocator::load_info() {
    ...
    while (ifs >> i >> a >> f >> s) {  // free, alloc, size
        infos.emplace_back(i, a, f, s);
    }
    ifs.close();
    return true;
}

void HeuristicAllocator::dump_heuristic_result() {
    ...
    for (int i = 0; i < infos.size(); i++) {
        ofs << infos[i].id << "\t" <<heuristic_address[i].first << "\n";
    }
    ofs.close();
}
